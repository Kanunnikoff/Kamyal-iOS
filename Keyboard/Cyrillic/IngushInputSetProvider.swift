...
public struct IngushInputSetProvider {
    ...
    public var alphabeticInputSet: KeyboardLayout.InputSet {
        ...
        KeyboardLayout.InputSet(rows: [
            // В Unicode существует формальная строчная палочка `ӏ`, поэтому
            // автоматическое преобразование всей строки меняет `Ӏ` вместе с
            // обычными буквами. Задаём оба регистра явно, чтобы палочка оставалась
            // одной и той же буквой U+04C0 при любом состоянии Shift.
            .init(
                lowercased: "йцукенгшӀзх",
                uppercased: "ЙЦУКЕНГШӀЗХ"
            ),
